...
from qube.src.api.swagger_models.omartestpy3 import omartestpy3Model # noqa: ignore=I100
from qube.src.api.swagger_models.omartestpy3 import omartestpy3ModelPost # noqa: ignore=I100
from qube.src.api.swagger_models.omartestpy3 import omartestpy3ModelPostResponse # noqa: ignore=I100
from qube.src.api.swagger_models.omartestpy3 import omartestpy3ModelPut # noqa: ignore=I100
...
from qube.src.api.swagger_models.response_messages import (
    ...
    put_response_msgs)
from qube.src.commons.error import omartestpy3ServiceError
...
class omartestpy3ItemController(Resource):
    @swagger.doc(
        {
            'tags': ['omartestpy3'],
            'description': 'omartestpy3 get operation',
    ...
    def get(self, authcontext, entity_id):
        """gets an omartestpy3 item that omar has changed
        """
        try:
            LOG.debug("Get details by id %s ", entity_id)
            data = omartestpy3Service(authcontext['context'])\
                .find_by_id(entity_id)
            clean_nonserializable_attributes(data)
        except omartestpy3ServiceError as e:
            LOG.error(e)
        ...
        return omartestpy3Model(**data), 200

    @swagger.doc(
        {
            'tags': ['omartestpy3'],
            'description': 'omartestpy3 put operation',
    ...
    def put(self, authcontext, entity_id):
        """
        updates an omartestpy3 item
        """
        try:
            model = omartestpy3ModelPut(**request.get_json())
            context = authcontext['context']
            omartestpy3Service(context).update(model, entity_id)
            return EMPTY, 204
        except omartestpy3ServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
    ...
    @swagger.doc(
        {
            'tags': ['omartestpy3'],
            'description': 'omartestpy3 delete operation',
    ...
    def delete(self, authcontext, entity_id):
        """
        Delete omartestpy3 item
        """
        try:
            omartestpy3Service(authcontext['context']).delete(entity_id)
            return EMPTY, 204
        except omartestpy3ServiceError as e:
            LOG.error(e)
            return ErrorModel(**{'error_code': str(e.errors.value),
...
class omartestpy3Controller(Resource):
    @swagger.doc(
        {
            'tags': ['omartestpy3'],
            'description': 'omartestpy3 get operation',
    ...
    def get(self, authcontext):
        """
        gets all omartestpy3 items
        """
        LOG.debug("Serving  Get all request")
        list = omartestpy3Service(authcontext['context']).get_all()
    ...
    @swagger.doc(
        {
            'tags': ['omartestpy3'],
            'description': 'omartestpy3 create operation',
    ...
    def post(self, authcontext):
        """
        Adds a omartestpy3 item.
        """
        try:
            model = omartestpy3ModelPost(**request.get_json())
            result = omartestpy3Service(authcontext['context'])\
                .save(model)

            response = omartestpy3ModelPostResponse()
