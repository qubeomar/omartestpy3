...
class omartestpy3Service:
    ...
    def find_by_id(self, entity_id):
        ...
        data = omartestpy3.query.get(entity_id)
        if data is None:
            raise omartestpy3ServiceError(
                'omartestpy3 {} not found'.format(entity_id),
    ...
    def get_all(self):
        list = []
        data = omartestpy3.query.filter(
            omartestpy3.tenantId == self.auth_context.tenant_id)
    ...
    def update(self, model, entity_id):
        ...
        record = omartestpy3.query.get(entity_id)  # omartestpy3 is a mongo class
        if record is None:
            raise omartestpy3ServiceError(
                'omartestpy3 {} not found'.format(entity_id),
                ErrorCodes.NOT_FOUND)
